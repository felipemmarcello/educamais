...
const ScienceQuiz = () => {
  ...
  useEffect(() => {
    const checkAnsweredBefore = async () => {
      if (user) {
        ...
        const responseSnapshot = await getDocs(collection(db, "userScienceResponses"));
  ...
  const handleSubmit = async () => {
    ...
    const userResponseQuery = collection(db, "userScienceResponses");
    ...
    await addDoc(collection(db, "userScienceResponses"), {
      userId: user.uid,
      ...
      <Box>
        <Box sx={{ display: 'flex', justifyContent: 'center', mt: 4, mb: 2, marginBottom: '3%' }}>
          <Typography variant="h3" gutterBottom>
            <TextWithColor subject="science" text={subject} />
          </Typography>
        </Box>
