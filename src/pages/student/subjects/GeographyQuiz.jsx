...
const GeographyQuiz = () => {
  ...
  useEffect(() => {
    const checkAnsweredBefore = async () => {
      if (user) {
        ...
        const responseSnapshot = await getDocs(collection(db, "userGeographyResponses"));
  ...
  const handleSubmit = async () => {
    ...
    const userResponseQuery = collection(db, "userGeographyResponses");
    ...
    await addDoc(collection(db, "userGeographyResponses"), {
      userId: user.uid,
      ...
      <Box>
        <Box sx={{ display: 'flex', justifyContent: 'center', mt: 4, mb: 2, marginBottom: '3%' }}>
          <Typography variant="h3" gutterBottom>
            <TextWithColor subject="geography" text={subject} />
          </Typography>
        </Box>
