...
const EnglishQuiz = () => {
  ...
  useEffect(() => {
    const checkAnsweredBefore = async () => {
      if (user) {
        ...
        const responseSnapshot = await getDocs(collection(db, "userEnglishResponses"));
  ...
  const handleSubmit = async () => {
    ...
    const userResponseQuery = collection(db, "userEnglishResponses");
    ...
    await addDoc(collection(db, "userEnglishResponses"), {
      userId: user.uid,
      ...
      <Box>
        <Box sx={{ display: 'flex', justifyContent: 'center', mt: 4, mb: 2, marginBottom: '3%' }}>
          <Typography variant="h3" gutterBottom>
            <TextWithColor subject="english" text={subject} />
          </Typography>
        </Box>
